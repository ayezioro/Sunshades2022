...
1:  ["_SurfaceOrPergola_", "0= Device optimised for period, will give the horizontal or tilted surface over the top of the window,"\
    " or the cut profile device on a provided shading surface. 1= Pergola with fins. Default is 0."],
2:  ["_window", "A Surface or Brep representing a window to be used for shading design.  This can also be a list of Surfaces of Breps."],
3:  ["_numOfShds_", "The number of shades to generated for each glazed surface."],
4:  ["_udiv_", "Number of row divisions of the window. Used for choosing the lower and higher rows you want to protect. Default is 1."],
5:  ["_sunVectors", "Output of Ladybug sunPath component."],
6:  ["context_", "Breps/surfaces that you want to account for as blocking objects. "],
7:  ["shadeSurface_", "An optional shade surface representing a 2D area under consideration for shading. "],
8:  ["---------------", "---------------"],
9:  ["_shdSrfAngle_", "In case NO shadeSurface is provided a plane over the window will be used as base for the calculation. " \
    "In this case you can provide the angle of this plane. Default is 0.0."],
10: ["_shdSrfShift_", "In case NO shadeSurface is provided a plane over the window will be used as base for the calculation. " \
    "In this case you can provide a shift distance from top of the window. Default is 0.01" ],
11:  ["_numPergolaFins_", "Number of pergola fins. Default is 10."],
12: ["_finsAngle_", "Angle for pergola fins. Default is 45."],
13: ["---------------", "---------------"],
14: ["_delaunayHeight_", "Distance from base curve and top intersection points. Used by the Delauney Mesh component. Default is 5."],
15: ["_offsetFactor_", "VERY important input!! The offset factor for the ConvexHull curve. Will be used for the Delauneay mesh. Default is 40."],
16: ["_cullRes_", "Resolution for culling points. 0=Don't cull, 1= Regular cull, 2= Cull a lot. Check the final surface for quality of results, 3= Extreme cull. Be carefull with the results. Default is 1."],
17: ["sunshds_", "Calculate sunshades according to: 0=behave as classic sunshades, 1= all shades the same"],
}
...
if (_SurfaceOrPergola_ == 0 or _SurfaceOrPergola_ == 2): # and shadeSurface_ == None:
    for input in range(1, numInputs):
...
def loopRowPoints_ORG(uPoints, udiv, numOfShds, sunshds):
    row1 = 0               # First row of points. Usually the bottom line
    start = row1 * udiv
    end   = start + udiv
    groupPoints = []
    ...
    for i in range(1, numOfShds + 1, 1): # Start from row 1. Row 0 is the bottom one.
        ...
        if sunshds == 1:
            start = start1# + udiv + 1         # This is the part that takes the lowerst row (or not) for calculation
            end   = end1#   + udiv + 1
        #elif sunshds == 0:
        #    start = start1# + udiv + 1         # This is the part that takes the lowerst row (or not) for calculation
        #    end   = end1#   + udiv + 1
...
def loopRowPoints(uPoints, udiv, numOfShds, sunshds):
    row1 = 0               # First row of points. Usually the bottom line
    start = row1 * udiv
    end   = start + udiv
    groupPoints = []
    ...
    for i in range(1, numOfShds + 1, 1): # Start from row 1. Row 0 is the bottom one.
        ...
        if sunshds == 0:
            st1 = (numOfShds * udiv + numOfShds)
            en1 = (numOfShds * udiv + numOfShds) + udiv
            groupPoints.append((start, end, st1, en1))
        else:
            groupPoints.append((start, end, start1, end1))
...
def finalSurfStuff(cullPts, delaunayHeight, offsetFactor, shadeSurface):
    ...
    ##print 'At end of surf', type(finalSrf)
...
def main(_numPergolaFins_, _shdSrfShift_):
    ...
    if sunshds_    == None:                               sunshds           = 1
    else:                                                 sunshds           = sunshds_ 
    
    # points
    pointsOnWindow, uPoints = pointsOfWindow(window, udiv, numOfShds)

    rowPoints, groupPoints = loopRowPoints(uPoints, udiv, numOfShds, sunshds)  # rowPoints is a list of lists. The output looks like IronPython.Runtime.List

    ##print 'rowPts ', rowPoints
    finalSrf      = []
    allCullPts    = []
    allptsContext = []
    
    sunShadeCullPts             = DataTree[System.Object]()                    # make a DataTree for Object
    
    #print 'lenrowpts ', len(rowPoints)
    #for pt in rowPoints:
    #    print pt[2]
    ...
    for i in range(len(rowPoints)):    # List or pair of rows index points (Upoints) to be analysed later on
        ...
        if cullPts == None:   # Some special cases there are point but no surface is created. SO just to be on the safe side
            ...
            # #####################################################################################
            path  = GH_Path(i)                            #Path for each individual curve
            path1 = GH_Path(i)                            #Path for each row of points
            sunShadeCullPts.AddRange(cullPts, path)      #Append to each row the relevant curves
            # #####################################################################################



    ##return finalSrf, allCullPts, allptsContext, pointsOnWindow, uPoints
    return finalSrf, allCullPts, allptsContext, pointsOnWindow, uPoints, rowPoints, groupPoints, sunShadeCullPts, finalSrf_TMP
...
if checkData == True:
    ...
    if result != -1:
        ...
        finalSrf, cullPts, ptsContext, pointsOnWindow, uPoints, rowPoints, groupPoints, sunShadeCullPts, finalSrf_TMP = result
else:
